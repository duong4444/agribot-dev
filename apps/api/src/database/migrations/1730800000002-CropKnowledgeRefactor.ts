...
/**
 * Migration: Crop Knowledge Refactor với Weighted FTS
 * 
 * Tạo table mới và functions cho Layer 1 FTS với chiến lược:
 * - loai_cay: Weight A (highest) 
 * - tieu_de_chunk: Weight B (high)
 * - chu_de_lon: Weight C (medium)
 * - noi_dung: Weight D (default)
 */
export class CropKnowledgeRefactor1730800000002 implements MigrationInterface {
  name = 'CropKnowledgeRefactor1730800000002';

  public async up(queryRunner: QueryRunner): Promise<void> {
    console.log('🌾 Starting Crop Knowledge Refactor migration...');
    ...
    console.log('✅ Created crop_knowledge_chunks table');
    ...
    await queryRunner.query(`
      ...
      BEGIN
        ...
        -- In production, you might want to add unaccent() here
    ...
    console.log('✅ Created generate_crop_chunk_tsvector function');
    ...
    console.log('✅ Created trigger for search vector');
    ...
    console.log('✅ Created search_crop_knowledge_fts function');
    ...
    console.log('✅ Created search_crop_by_name_and_topic function');
    ...
    console.log('🎉 Crop Knowledge Refactor migration completed!');
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    console.log('Rolling back Crop Knowledge Refactor migration...');

    // Drop view
    await queryRunner.query(`DROP VIEW IF EXISTS crop_knowledge_statistics`);

    // Drop functions
    await queryRunner.query(`DROP FUNCTION IF EXISTS search_crop_by_name_and_topic`);
    await queryRunner.query(`DROP FUNCTION IF EXISTS search_crop_knowledge_fts`);
    await queryRunner.query(`DROP FUNCTION IF EXISTS update_crop_chunk_search_vector`);
    await queryRunner.query(`DROP FUNCTION IF EXISTS generate_crop_chunk_tsvector`);
    await queryRunner.query(`DROP FUNCTION IF EXISTS normalize_vietnamese_text`);

    // Drop trigger
    await queryRunner.query(`
      DROP TRIGGER IF EXISTS trigger_update_crop_chunk_search_vector 
      ON "crop_knowledge_chunks"
    `);

    // Drop indexes
    await queryRunner.query(`DROP INDEX IF EXISTS "idx_chunk_id_unique"`);
    await queryRunner.query(`DROP INDEX IF EXISTS "idx_crop_knowledge_composite"`);
    await queryRunner.query(`DROP INDEX IF EXISTS "idx_crop_knowledge_search_vector"`);
    await queryRunner.query(`DROP INDEX IF EXISTS "idx_crop_knowledge_chu_de"`);
    await queryRunner.query(`DROP INDEX IF EXISTS "idx_crop_knowledge_loai_cay"`);

    // Drop table
    await queryRunner.query(`DROP TABLE IF EXISTS "crop_knowledge_chunks"`);

    console.log('✅ Crop Knowledge Refactor migration rolled back');
  }
}
